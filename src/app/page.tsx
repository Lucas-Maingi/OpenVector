...
export default function Landing() {
  ...
  return (
    <div className="w-full bg-[#020617] min-h-screen">
      ...
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden pt-20">
        ...
        <div className="max-w-5xl mx-auto px-6 relative z-10 text-center w-full mt-10">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-900/50 border border-slate-700/50 text-slate-300 text-sm font-mono mb-8 shadow-2xl backdrop-blur-md"
          >
            <Sparkles className="w-4 h-4 text-purple-400" />
            <span className="text-purple-400 font-bold">Aletheia OSINT v2.4</span> Engine Online
          </motion.div>
          ...
          <motion.h1
            ...
            className="text-5xl md:text-7xl font-serif font-bold text-white tracking-tight leading-[1.1] mb-6 drop-shadow-2xl"
          >
          ...
          <motion.p
            ...
            className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-12 leading-relaxed font-sans"
          >
          ...
          >
            {/* Glowing Backdrop */}
            <div className="absolute -inset-1.5 bg-gradient-to-r from-purple-600 via-indigo-500 to-lime-400 rounded-2xl blur-lg opacity-30 group-hover:opacity-60 transition duration-1000 group-hover:duration-300 pointer-events-none" />
            
            <form onSubmit={handleSearch} className="relative flex flex-col md:flex-row items-stretch md:items-center bg-slate-950/80 backdrop-blur-2xl border border-slate-700/80 rounded-2xl p-2.5 shadow-2xl overflow-hidden gap-2">
              <div className="flex items-center flex-1 px-4 py-2 relative">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-none text-white text-lg px-4 focus:outline-none focus:ring-0 placeholder:text-slate-500 font-mono"
                />
              </div>
              <button 
                type="submit"
                className="flex items-center justify-center gap-2 bg-white hover:bg-slate-200 text-slate-950 px-8 py-4 rounded-xl font-bold transition-all transform md:hover:scale-[1.02] shadow-[0_0_20px_rgba(255,255,255,0.2)] shrink-0 h-full"
              >
      ...
      <section id="agents" className="py-24 bg-slate-950/50 relative border-t border-slate-800/40 backdrop-blur-3xl">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div
                ...
                className="group relative p-8 rounded-2xl bg-[#020617] border border-slate-800/60 hover:border-slate-700 transition-colors shadow-2xl overflow-hidden"
              >
                ...
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 ${agent.bg} ${agent.color} shadow-inner border border-slate-800/50`}>
                  <agent.icon className="w-6 h-6" />
                </div>
                <h3 className="text-xl font-serif font-bold text-white mb-3 flex items-center gap-2">
                  {agent.title}
                  <span className="text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-slate-800 text-slate-400 font-mono">Active</span>
                </h3>
                <p className="text-slate-400 text-sm leading-relaxed">{agent.desc}</p>
                <div className="mt-6 pt-6 border-t border-slate-800/60 flex items-center justify-between text-xs font-mono text-slate-500">
      ...
      <section id="proof" className="py-32 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative">
            {/* Recursive Mock UI */}
            <div className="relative z-10 rounded-xl overflow-hidden border border-slate-700/50 shadow-2xl shadow-purple-500/10 bg-slate-900/80 backdrop-blur-xl aspect-square lg:aspect-video transform lg:-rotate-y-12 perspective-1000">
              <div className="h-8 border-b border-slate-700/50 bg-slate-950/80 flex items-center px-4 gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-slate-700" />
                <div className="w-2.5 h-2.5 rounded-full bg-slate-700" />
                <div className="w-2.5 h-2.5 rounded-full bg-slate-700" />
                <div className="ml-auto text-xs text-slate-500 font-mono">live-feed.sh</div>
              </div>
      ...
      {/* Pricing Comparison */}
      <section id="pricing" className="py-24 bg-slate-950 border-t border-slate-800/40">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-white mb-4">The new standard in OSINT.</h2>
            <p className="text-slate-400 text-lg">Compare Aletheia against traditional investigation methods.</p>
          </div>

          <div className="max-w-4xl mx-auto overflow-hidden rounded-2xl border border-slate-800/60 bg-[#020617] shadow-2xl">
            <div className="grid grid-cols-3 bg-slate-900/50 border-b border-slate-800/60 p-6 items-center">
              <div className="font-semibold text-slate-400 font-mono text-sm uppercase tracking-wider">Capabilities</div>
              <div className="text-center font-semibold text-slate-400 font-mono text-sm uppercase tracking-wider">Private Investigators</div>
              <div className="text-center font-bold text-purple-400 text-lg flex items-center justify-center gap-2">
            ...
            {[
              ...
              <div key={i} className={`grid grid-cols-3 p-6 border-b border-slate-800/60 last:border-0 items-center ${row.focus ? 'bg-purple-500/5' : ''}`}>
                <div className="font-medium text-slate-300 text-sm md:text-base">{row.label}</div>
                <div className="text-center text-slate-500 font-mono text-sm">{row.pi}</div>
                <div className={`text-center font-bold ${row.focus ? 'text-lime-400' : 'text-slate-200'}`}>{row.al}</div>
              </div>
            ))}
          </div>
